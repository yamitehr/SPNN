...
def smooth_paste(im1, im2, topleft, h, w, smoothness=1.0):
    """Gaussian-feathered alpha blend: weight=1 inside the box (top:top+h,
    left:left+w), Gaussian falloff outside with σ proportional to box
    size scaled by `smoothness`. Returns weight*im1 + (1-weight)*im2."""
    H, W = im1.shape[-2:]
    top, left = topleft
    ...
    dy = (top - yy).clamp(min=0) + (yy - (top + h - 1)).clamp(min=0)
    dx = (left - xx).clamp(min=0) + (xx - (left + w - 1)).clamp(min=0)
    weight = torch.exp(-0.5 * ((dy / sy) ** 2 + (dx / sx) ** 2))
...
class Diffusion(object):
    ...
    def simplified_ddnm_plus(self, model):
        ...
        for x_orig, classes in pbar:
            ...
            # pixel-paste region, in 256-px coords.
            target_box = None
            target_cls = None
            ...
            # bb-edit preprocessing: cut the object from x_orig, paste at
            # the mirrored position (point reflection through image
            # center), fill the source hole with bilinear interpolation
            # from the 4 boundary strips. Then update target_box so the
            # diffusion's paste step lands on the moved object.
            if target_box is not None:
                S = config.data.image_size
                bx1, by1, bx2, by2 = target_box
                px1 = max(0, int(bx1)); py1 = max(0, int(by1))
                px2 = min(S, int(np.ceil(bx2))); py2 = min(S, int(np.ceil(by2)))
                h, w = py2 - py1, px2 - px1
                if h > 0 and w > 0:
                    obj = x_orig[..., py1:py2, px1:px2].clone()
                    mx1, my1, mx2, my2 = S - px2, S - py2, S - px1, S - py1
                    ty = max(0, py1 - 1); by_ = min(S - 1, py2)
                    lx = max(0, px1 - 1); rx = min(S - 1, px2)
                    top   = x_orig[..., ty:ty + 1,    px1:px2]
                    bot   = x_orig[..., by_:by_ + 1,  px1:px2]
                    left  = x_orig[..., py1:py2,      lx:lx + 1]
                    right = x_orig[..., py1:py2,      rx:rx + 1]
                    iy = torch.linspace(0, 1, h, device=x_orig.device).view(1, 1, h, 1)
                    ix = torch.linspace(0, 1, w, device=x_orig.device).view(1, 1, 1, w)
                    x_orig[..., py1:py2, px1:px2] = 0.5 * (
                        (1 - iy) * top + iy * bot
                        + (1 - ix) * left + ix * right
                    )
                    # Soft-paste the chair at the mirrored box. Plateau
                    # = bbox minus a thin (~10%) margin so chair pixels
                    # are preserved everywhere except a thin ring near
                    # the bbox edge; σ kept very small (~2% of plateau)
                    # so the fade completes inside that ring and weight
                    # at the bbox boundary is ~0 (output continuous).
                    donor = x_orig.clone()
                    donor[..., my1:my2, mx1:mx2] = obj
                    mh, mw = my2 - my1, mx2 - mx1
                    mh_in = max(1, mh - max(2, mh // 10))
                    mw_in = max(1, mw - max(2, mw // 10))
                    mtop_in = my1 + (mh - mh_in) // 2
                    mleft_in = mx1 + (mw - mw_in) // 2
                    x_orig = smooth_paste(
                        donor, x_orig,
                        topleft=(mtop_in, mleft_in),
                        h=mh_in, w=mw_in,
                        smoothness=0.02)
                    target_box = (float(mx1), float(my1), float(mx2), float(my2))
                ...
                for step_idx, (i, j) in enumerate(tqdm.tqdm(time_pairs)):
                    ...
                    if j < i:  # normal sampling
                        ...
                        if lambda_t != 0.0 and nlbp_error > args.nlbp_stop_cond:
                            #y_noise = y + torch.randn_like(y)*0.025
                            y_noise = y
                            y_tar, z_tar = A(Ap(y_noise), return_latents=True)
                        ...
                        if target_box is not None:
                            ...
                            # Shrink the plateau to the central 1/4 and use
                            # σ ≈ h/4 so (a) the bbox edges sit inside the
                            # Gaussian falloff (no rectangular halo) and
                            # (b) the tail decays to ~0 by one bbox-size
                            # out (no background leak from x0_t_hat).
                            h_in = max(1, h // 4)
                            w_in = max(1, w // 4)
                            top_in = top + (h - h_in) // 2
                            left_in = left + (w - w_in) // 2
                            x0_t = smooth_paste(
                                x0_t_hat, x0_t,
                                topleft=(top_in, left_in),
                                h=h_in, w=w_in,
                                smoothness=1.0)
                        ...
                        xt_next = at_next.sqrt() * x0_t + c2 * et + sigma_t * torch.randn_like(x0_t)
            ...
            # bb-edit: save the (possibly-edited) x_orig that was actually
            # fed into A — i.e. with the object moved to its mirrored
            # position and the source hole interpolated.
            tvu.save_image(
                orig, os.path.join(self.args.image_folder,
                                   f"x_orig_{idx_so_far}.png"))
            # Save result grid
