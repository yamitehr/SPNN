...
# SPNN image reconstruction loss: ||A†(A(x)) - x||²
# Trains the whole SPNN chain (s/t/r networks + affine adapter) to be a good
# right-inverse. Helpful for downstream DDNM. 0.0 disables.
parser.add_argument('--lambda_img_rec', type=float, default=0.0,
                    help='Weight for image reconstruction loss '
                         '||SPNN_pinv(SPNN(x)) - x||². '
                         'Trains the whole SPNN chain to be a good right-'
...
def main():
  ...
  def train(epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
      ...
      # Snapshot dense outputs (single-scale → first/only entry of each tuple)
      # before the GT-position gather, so the image-rec loss can call SPNN.pinv
      # on the full [B, num_classes+4, H, W] tensor.
      hmap_dense, regs_dense, wh_dense = hmap[0], regs[0], w_h_[0]
      ...
      img_rec_val = 0.0
      if cfg.arch == 'spnn' and cfg.lambda_img_rec > 0:
        # Image reconstruction loss: ||SPNN_pinv(SPNN(x)) - x||²
        # Uses the wrapper's pinv (inverts affine + mixer, then SPNN.pinv with
        # r-net latent reconstruction). Trains the whole chain to round-trip.
        m = model.module if hasattr(model, 'module') else model
        x_inv = m.pinv(hmap_dense, regs_dense, wh_dense, latents=None)
        img_rec_loss = (x_inv - batch['image']).pow(2).mean()
        loss = loss + cfg.lambda_img_rec * img_rec_loss
        img_rec_val = img_rec_loss.item()
      ...
      if batch_idx % cfg.log_interval == 0:
        ...
        if cfg.lambda_img_rec > 0:
          msg += ' img_rec= %.5f' % img_rec_val
        ...
        if cfg.lambda_img_rec > 0:
          summary_writer.add_scalar('img_rec_loss', img_rec_val, step)
        ...
        if wandb_run is not None:
          ...
          if cfg.lambda_img_rec > 0:
            wandb_log['train/img_rec_loss'] = img_rec_val
