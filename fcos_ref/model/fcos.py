...
class FCOS_SPNN(nn.Module):
    """FCOS with SPNN backbone.

    Uses SPNNBackbone (single-scale [48, 16, 16]) + FPN_SPNN neck
    to produce multi-scale features, then the standard FCOS head.
    """
    ...
    def __init__(self, config=None):
        ...
        self.head = ClsCntRegHead(
            config.fpn_out_channels, config.class_num,
            config.use_GN_head, config.cnt_on_reg, config.prior,
        )
        self.config = config
    ...
    def train(self, mode=True):
        ...
        if getattr(self.config, 'freeze_stage_1', False):
            self.backbone.freeze_stages(1)
            print("INFO===>success frozen SPNN backbone stage1")

    def forward(self, x):
        feat = self.backbone(x)        # [B, 48, 16, 16]
        all_P = self.fpn(feat)          # [P3, P4, P5, P6, P7]
        cls_logits, cnt_logits, reg_preds = self.head(all_P)
        return [cls_logits, cnt_logits, reg_preds]
...
class FCOSDetector(nn.Module):
    def __init__(self,mode="training",config=None):
        ...
        if config is None:
            config=DefaultConfig
        self.mode=mode
        if getattr(config, 'backbone_type', 'resnet') == 'spnn':
            self.fcos_body = FCOS_SPNN(config=config)
        else:
