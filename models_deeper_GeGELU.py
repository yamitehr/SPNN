...
class _GeGLU(nn.Module):
    """Gated linear unit with GELU gate. Splits the channel dim of the input
    into two equal halves (a, b) and returns a * GELU(b), i.e. (B, 2C, H, W)
    → (B, C, H, W). The multiplicative gate adds per-pixel channel-direction
    interaction that a plain pointwise activation cannot express, which is
    what we want inside s/t/r so the head can do per-cell class discrimination
    without relying on the single shared W rotation.
    """
    def forward(self, x):
        a, b = x.chunk(2, dim=1)
        return a * F.gelu(b)
...
class _UNet3Skip(nn.Module):
    ...
    Activations are GeGLU: every Conv → GN → activation triple produces 2× the
    target channels, GroupNorm normalizes the doubled tensor, and GeGLU splits
    and gates back to the target dim. Final conv is zero-init so the block
    warm-starts as identity, matching the Sequential branches' init contract.
    """
    def __init__(self, in_ch, out_ch, h1, h2, h3, h4):
        super().__init__()
        def _enc(ci, co, stride):
            g = _gn_groups(2 * co)
            return nn.Sequential(
                nn.Conv2d(ci, 2 * co, 3, stride=stride, padding=1),
                nn.GroupNorm(g, 2 * co), _GeGLU(),
                nn.Conv2d(co, 2 * co, 3, padding=1),
                nn.GroupNorm(g, 2 * co), _GeGLU())
        def _up(ci, co):
            g = _gn_groups(2 * co)
            return nn.Sequential(
                nn.ConvTranspose2d(ci, 2 * co, 4, 2, 1),
                nn.GroupNorm(g, 2 * co), _GeGLU())
        def _dec(c):  # input channels are 2*c after skip-concat
            g = _gn_groups(2 * c)
            return nn.Sequential(
                nn.Conv2d(2 * c, 2 * c, 3, padding=1),
                nn.GroupNorm(g, 2 * c), _GeGLU())
        self.enc1, self.enc2 = _enc(in_ch, h1, 1), _enc(h1, h2, 2)
        self.enc3, self.enc4 = _enc(h2, h3, 2), _enc(h3, h4, 2)
        self.up3, self.up2, self.up1 = _up(h4, h3), _up(h3, h2), _up(h2, h1)
        self.dec3, self.dec2, self.dec1 = _dec(h3), _dec(h2), _dec(h1)
