...
class ConvMLP(nn.Module):
    def __init__(self, in_ch, out_ch, scale_bound, hidden_ch, img_size: int = 32,
                 ...
                 final_log_scale_init=None):
        """Optional learnable per-output-channel affine, applied INSIDE forward
        AFTER `self.net(x)`:

          - final_bias_init  (only valid when scale_bound is None — i.e. on
            t/r networks): sequence of length `out_ch`. Constructs a
            learnable nn.Parameter `self.final_bias` of shape (1, out_ch, 1, 1)
            initialized to these values; added to the network output as
            `out = net(x) + self.final_bias`.

          - final_log_scale_init (only valid when scale_bound is not None —
            i.e. on s networks): sequence of length `out_ch`. Constructs a
            learnable nn.Parameter `self.final_log_scale` of shape
            (1, out_ch, 1, 1). Multiplies the post-exp s output by
            exp(±log_scale) (sign follows the `neg` flag in forward), so
            s_full(x1, neg=True) = 1 / s_full(x1, neg=False) — coupling
            inverse stays exact.

        When None (default), no extra param is constructed; the module is
        ...
        # Optional learnable per-output-channel affine (lives ON THIS MODULE
        # — see docstring above for semantics and bijectivity argument).
        self.final_bias = None
        self.final_log_scale = None
        if final_bias_init is not None:
            assert scale_bound is None, (
                "final_bias_init only valid for additive (t/r) networks; "
                f"got scale_bound={scale_bound}")
            init = torch.as_tensor(final_bias_init, dtype=torch.float32)
            assert init.numel() == out_ch, (
                f"final_bias_init must have length out_ch={out_ch}, "
                f"got {init.numel()}")
            self.final_bias = nn.Parameter(init.view(1, out_ch, 1, 1).clone())
        if final_log_scale_init is not None:
            assert scale_bound is not None, (
                "final_log_scale_init only valid for multiplicative (s) "
                f"networks; got scale_bound={scale_bound}")
            init = torch.as_tensor(final_log_scale_init, dtype=torch.float32)
            assert init.numel() == out_ch, (
                f"final_log_scale_init must have length out_ch={out_ch}, "
                f"got {init.numel()}")
            self.final_log_scale = nn.Parameter(init.view(1, out_ch, 1, 1).clone())
    ...
    def forward(self, x, neg=False):
        ...
        if self.scale_bound is not None:
            ...
            if neg:
                x = -x
            x = x.exp()
            if self.final_log_scale is not None:
                # Per-channel multiplicative gain on the s output.  neg=True
                # flips the sign of log_scale so that
                #   s_full(x1, True) = (1/s_inner) * exp(-log_scale)
                #                    = 1 / s_full(x1, False)
                # — coupling-layer inverse round-trip stays exact.
                sign = -1.0 if neg else 1.0
                x = x * torch.exp(sign * self.final_log_scale)
        else:
            # t and r networks: unbounded additive output.
